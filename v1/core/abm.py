...
class Sim:
    def __init__(self,sim,city,p_factor=1.0,scenario='ia',n=None,destins=None,speedup=60,loadpath=True,fresh_place=False):
        ...
        self.p.init_EM(fresh=fresh_place)
        ...
        if loadpath:
            self.p.init_path(fresh=fresh_place)
        ...
        if loadpath:
            self.init_EA(fresh=fresh_place)
            
    def init_file(self, scenario=None):
        """Make agents file that is specific to scenario and the list of destin nodes."""
        ...
        if self.p.destins:
            ...
            self.df_agents_file = '{0}/{1}-n-{2}-d-{3}'.format(self.city_folder,self.scenario,self.n,self.hash)
            self.video_file = '{0}.mp4'.format(self.df_agents_file)
            return True
        else:
            raise('{0}: Agent file could not be made because destin nodes are not available!'.format(self.city))
            sys.stdout.flush()
            return False
    ...
    def save_agents(self):
        ...
        print '{0}: Initialised agents pickled.'.format(self.city)
        sys.stdout.flush()
    ...
    def load_agents(self):
        ...
        if os.path.isfile(fname):
            ...
            print '{0}: Initialised agents unpickled.'.format(self.city)
            sys.stdout.flush()
            return True
        else:
            return False
    ...
    def load_results(self):
        ...
        if os.path.isfile(df_fname):
            ...
            print '{0}: Time results could not be loaded.'.format(self.city)            
            success = False
        ...
        if os.path.isfile(df_fname):
            ...
            print '{0}: Agent density profile of each agent could not be loaded.'.format(self.city)            
            success = False    
        ...
        if os.path.isfile(df_fname):
            ...
            print '{0}: Agent density profile per timestep could not be loaded.'.format(self.city)            
            success = False    
        ...
        if os.path.isfile(df_fname):
            ...
            print '{0}: Agent total presence density map could not be loaded.'.format(self.city)            
            success = False    

        print '{0}: All results loaded.'.format(self.city)
        sys.stdout.flush()
    ...
    def init_agents(self, fresh=False):
        ...
        if not self.load_agents() or fresh==True:
            ...
            print '{0}: {1}% of agents ({2} of {3}) had to be re-created!'.format(self.city,e*100/float(self.n),e,self.n)
            sys.stdout.flush()
    ...
    def run_sim(self,fresh=False,video=True,live_video=False,bitrate=4000,fps=20):
        ...
        def sim_init():
            print '{0}: Starting this simulation...'.format(self.city)
            sys.stdout.flush()
        ...
        def sim_animate(i):
            if not self.sim_complete:
                ...
                point.set_array(np.array(self.D)[self.S])

                agents_text.set_text('T:{0}, A:{1}'.format(i,agents_left))
                # Print out the progress
                if i%10==0 or agents_left == 0:
                    print '{0}: Frame {1} Agents {2} A0: Density {3} Time {4}...'.format(self.city,i,agents_left,self.D[0],self.T[0])
                    sys.stdout.flush()
                
                if agents_left == 0:
                    self.sim_complete = True
                    print '{0}: End of this simulation.'.format(self.city)
                    sys.stdout.flush()
                else:
        ...
        if video:
            ...
            print '{0}: Saving video to file {1}...'.format(self.city,self.video_file)
            sys.stdout.flush()
    ...
    def init_EA(self,fresh=False):
        """Function to initialise area of edges."""
        
        fname = '{0}.EA'.format(self.agents_file)        
        if os.path.isfile(fname) and fresh==False:
            file = open(fname, 'r')
            self.EA = pickle.load(file)
            file.close()
        else:
            # Calculate the minimum area so that at least one agent can fit through freely at all times
            EA_min = 1.0/self.f.pFlat
            # Calculate the minimum area for the destination edges so that all agents can travel at max velocity
            EA_all = float(self.n)/self.f.pFlat
            
            # Initialise area of edges
            self.EA = [None] * self.p.edges

            # Assign the widths to the edges
            for i in range(self.p.edges):
                self.EA[i] = self.p.DAM.data[i]*self.width[self.p.HAM.data[i]-1]
                
                # If the edge area is less than the minimum area (enough to handle one agent)
                if self.EA[i] < EA_min:
                    self.EA[i] = EA_min
                
                # If the edge is one of the destination edges, make sure all agents can fit in
                if i in self.p.all_destin_edges:
                    self.EA[i] = EA_all
    ...
    def density(self,edge,add=0):
        ...
        return self.df[self.scenario]*(self.N[edge]+add)/self.EA[edge]
    ...
    def et_figure(self,lognorm=False,count=100):
        ...
        plt.savefig('{0}-et.png'.format(self.df_agents_file), dpi=300)        
        plt.close(fig)
        return fig

    # Plot population distribution map
    def highlight_agents(self,agent_list,agent_weight,weight_label,fname,sample_size=1000):
        ...
        plt.savefig(fname.format(self.df_agents_file), dpi=300)
        plt.close(fig)
        return fig
