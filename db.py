...
class Query:
    ...
    def connect(self):
        try:
            try:
                con = psycopg2.connect("dbname={0} host=localhost user=bk12369 password=postgres".format(dbname))
            except:
                con = psycopg2.connect("dbname={0} host=localhost user=bharatkunwar".format(dbname))
            self.cur = con.cursor()
        except Exception, e:
            print 'Oh dear! There is something wrong with the connection information you have supplied for the database!', e
...
class Highway:
    def __init__(self,place,fresh=False):
        ...
        place: string or tuple (xmin, ymin, xmax, ymax)
            Name of the place to lookup highway graph for.
        fresh: boolean
        ...
        if os.path.isfile(fname) and not self.fresh == True:
            ...
            for way,osm_id,highway,oneway_tag in r:
                ...
                for d in destin_coord[oneway]:
                    if not self.boundary.shape.intersects(shapely.geometry.Point(d)):
    ...
    def oneway(self,oneway_tag):
        ...
        1 if oneway
        0 if both ways
        -1 if oneway in opposite direction'''
...
class Flood:
    def __init__(self,filename):
        ...
        ----------
            filename: String
                Location of geographical file which consists of binary flood map
        '''
    ...
    def coordinateIndex(self,c):
        '''
        Returns the index of x and y coordinate given that we know the origin and pixel resolution.
